...
from deepracing_ros.controls.path_server_spliner import SplinerPathServer
import threading
def call_line_service(getline_client : rclpy.client.Client, node : rclpy.node.Node, key : str):
    ...
    while not success:
        ...
        getlineresponse : deepracing_srvs.GetLine.Response | None = future.result()
...
def main(args=None):
    ...
    initialize_thread = threading.Thread(
        ...
        args=(raceline_np, innerbound_np, outerbound_np),
        # daemon=True
    )
