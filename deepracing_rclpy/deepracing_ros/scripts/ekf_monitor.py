...
class EKFMonitor(rclpy.node.Node):
    ...
    def motionDataCB(self, motion_data : deepracing_msgs.msg.TimestampedPacketMotionData):
        ...
        if np.linalg.norm(delta_pos, ord=2, axis=0)>10.0:
            ...
            state_to_pub.pose.pose.covariance = self.pose_cov.flatten().tolist()
            ...
            state_to_pub.twist.twist.covariance = self.twist_cov.flatten().tolist()
            ...
            state_to_pub.accel.accel.covariance = self.accel_cov.flatten().tolist()
