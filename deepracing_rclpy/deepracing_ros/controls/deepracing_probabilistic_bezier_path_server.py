...
class BezierPredictionPathServerROS(PathServerROS):
    def __init__(self):
        super(BezierPredictionPathServerROS, self).__init__(name="admiralnet_path_server")
        ...
        self.deltaT : float = 0.8825*(deltaT_param.get_parameter_value().double_value)
        ...
        with torch.no_grad():
            ...
            for i in range(self.gaussian_filtering_steps):
               ...
               # max_ca_deltas, _ = torch.max(ca_deltas, dim=1)
                ...
                boundary_scores = torch.clip( torch.exp(0.0*overall_max_distances.double()), 1E-32, 1.0)
