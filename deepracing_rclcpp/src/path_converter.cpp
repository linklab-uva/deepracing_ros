...
#include <deepracing_msgs/msg/path_raw.hpp>
...
class NodeWrapperPathConverter_ 
{
  ...
  public:
    NodeWrapperPathConverter_( const rclcpp::NodeOptions & options = (
     ...
     this->listener = this->node->create_subscription<deepracing_msgs::msg::PathRaw>("/predicted_path_raw", rclcpp::SensorDataQoS().keep_all(), std::bind(&NodeWrapperPathConverter_::pathCallback, this, std::placeholders::_1));
     this->publisher = this->node->create_publisher<nav_msgs::msg::Path>("/predicted_path", 10);
     
     
     
    }  
    rclcpp::Subscription<deepracing_msgs::msg::PathRaw>::SharedPtr listener;
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr publisher;
    std::shared_ptr<rclcpp::Node> node;
  private:
    void pathCallback(const deepracing_msgs::msg::PathRaw::SharedPtr raw_path)
    {
