...
def generate_launch_description():
    ...
    entries = [config_file,use_sim_time,tf_from_odom]
    ...
    entries.append(launch_ros.actions.Node(package='deepracing_rclcpp', name='f1_tf_updater', executable='tf_updater', output='screen', parameters=[LaunchConfiguration(config_file.name), {"use_sim_time" : LaunchConfiguration(use_sim_time.name), "tf_from_odom" : LaunchConfiguration(tf_from_odom.name)}], remappings=[("motion_data", "/f1_game/motion_data"), ("session_data", "/f1_game/session_data")]))
